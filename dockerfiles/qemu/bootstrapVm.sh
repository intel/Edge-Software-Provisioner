...
if [ ! -d /data/volumes ]; then
  if [ "${ISO:0:1}" != "/" ] || [ -z "$IMAGE" ]; then
    echo "/data/volumes not mounted: using -v to mount it"
    exit 1
  fi
fi
...
if [ "$DISK_DEVICE" == "scsi" ]; then
  ...
  FLAGS_DISK_IMAGE="-device ide-hd,bus=ide.0,unit=0,drive=hd,id=drive,bootindex=1 -drive file=${IMAGE},if=none,id=hd,cache=${IMAGE_CACHE},format=${IMAGE_FORMAT},index=1"
elif [ "$DISK_DEVICE" == "virtio" ]; then
  FLAGS_DISK_IMAGE="-device virtio-blk-pci,scsi=off,bus=pci.0,drive=hd,id=drive,bootindex=1 -drive file=${IMAGE},if=none,id=hd,cache=${IMAGE_CACHE},format=${IMAGE_FORMAT},index=1"
...
if [ "$NETWORK" == "bridge" ]; then
  ...
  FLAGS_NETWORK="-net nic,model=${NETWORK_DEVICE} -net tap,ifname=tap0,script=no"
  ...
  FLAGS_NETWORK="-net nic,model=${NETWORK_DEVICE} -net user ${REDIR}"
fi
echo "Using ${NETWORK}"
...
if [ "$VNC" == "tcp" ] && [ "$VIDEO" == "vnc" ]; then
  FLAGS_REMOTE_ACCESS="-vnc ${VNC_IP}:${VNC_ID} -vga cirrus"
elif [ "$VNC" == "reverse" ] && [ "$VIDEO" == "vnc" ]; then
  FLAGS_REMOTE_ACCESS="-vnc ${VNC_IP}:${VNC_PORT},reverse -vga cirrus"
elif [ "$VNC" == "sock" ] && [ "$VIDEO" == "vnc" ]; then
  FLAGS_REMOTE_ACCESS="-vnc unix:${VNC_SOCK} -vga cirrus"
elif [ "$VIDEO" == "spice" ] && [ -n "$SPICE_OPTIONS" ]; then
  FLAGS_REMOTE_ACCESS="-spice $SPICE_OPTIONS"
elif [ "$VIDEO" == "spice" ] && [ "$SPICE" == "tcp" ]; then
  FLAGS_REMOTE_ACCESS="-spice port=${SPICE_PORT},addr=${SPICE_IP},disable-ticketing -vga qxl"
elif [ "$VIDEO" == "spice" ] && [ "$SPICE" == "sock" ]; then
  FLAGS_REMOTE_ACCESS="-spice unix,addr=${SPICE_SOCK},disable-ticketing -vga qxl"
elif [ "$VIDEO" == "virgl" ]; then
  xhost +
  FLAGS_REMOTE_ACCESS="-device virtio-vga,virgl=on -display sdl,gl=on"
elif [ "$VIDEO" == "custom" ]; then
  FLAGS_REMOTE_ACCESS="${CUSTOM_VIDEO}"
else
